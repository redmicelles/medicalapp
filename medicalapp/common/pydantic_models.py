    ...
    def validate_input(cls, values):
        if not all(
            ...
            raise exceptions.APIException(
                detail={
                    "message": "Names should contain only letters and '-' !",
                    "status": 422,
                }
            )
        if values.password._secret_value != values.password2._secret_value:
            raise exceptions.APIException(
                detail={"message": "Password mismacth!", "status": 422}
            )
        if not match(PASSWORD_PATTERN, values.password._secret_value):
            raise exceptions.APIException(
                detail={"message": "Password is not valid!", "status": 422}
            )
        return values
...
class LoginRequest(BaseModel):
    model_config = dict(extra="forbid")
    email: EmailStr = Field(validation_alias=AliasPath("email"))
    password: SecretStr = Field(validation_alias=AliasPath("password"))
...
class CreateRecordRequest(BaseModel):
    ...
    def validate_input(cls, values):
        if not match(DOCTOR_PATTERN, values.doctor):
            raise exceptions.APIException(
                detail={"message": "Doctor's name is not valid", "status": 422}
            )
        return values
...
class UpdateRecordRequest(BaseModel):
    ...
    diagnosis: Optional[str] = Field(
        validation_alias=AliasPath("diagnosis"), max_length=500
    )
    treatment: Optional[str] = Field(
        validation_alias=AliasPath("treatment"), max_length=500
    )
...
class APIErrorModel(BaseModel):
    model_config = dict(extra="forbid")
    error: dict[str, Union[str, list, dict]] = Field(
        validation_alias=AliasPath("error")
    )
